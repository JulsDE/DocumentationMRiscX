...
in RISC-V-assembly, some custom tactic were created.
...
3. Now, there several situations that might come up:
  1) Sequential code sections
    * Use `S_SEQ` or some custom tactic to "peel off" the last instruction and inspect
  ...
  3) Loops
    * Apply `S_LOOP` by finding the condition `C` of the loop, the loop invariant `I`
      ...
      {ref "hoare-rules"}[the Hoare-rules]
