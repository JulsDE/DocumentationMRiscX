...
different levels. Details about this can be found in T
...
but this can
be replaced by actual `RISC-V` assembly code. More about this in the chapter
{ref "assembly"}[]
2. The Hoare-triple: This section consists of three subsections
      ...
      This has the structure of `l ↦ ⟨L_W | L_B⟩`, where
            * `l` represent the line where the program starts
            (where the {lean}`ProgramCounter` ({index}[PC]) points to
            before running the program).
            * `L\_W` is the white list, a set containing all the lines where
            the PC might point to *after*
            executing the program. So when we want to let the program run for
            one line.
            * `L\_B` is a white list. This set contains all the lines, which must
...
Let {index}[$`\mathbb{U}_{64}`] be the finite set of all natural numbers from $`0` to $`2^{64} - 1`.
